...
class SimpleSuite extends FunSuite {
  ...
  test("mapSet1 set works correctly") {
    val s1 = RTUSet.mapSet1(RTUSet.singletonSet(1), (x: Int) => x * -1)
    val s2 = RTUSet.mapSet1(RTUSet.singletonSet(3), (x: Int) => x * 10)
    assert(RTUSet.contains(s1,-1))
    assert(RTUSet.contains(s2,30))
  }

  test("mapSet2 set works correctly") {
    val s1 = RTUSet.mapSet2(RTUSet.singletonSet(1), (x: Int) => x * -1)
    val s2 = RTUSet.mapSet2(RTUSet.singletonSet(3), (x: Int) => x * 10)
    assert(RTUSet.contains(s1,-1))
    assert(RTUSet.contains(s2,30))
  }
